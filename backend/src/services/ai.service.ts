...
export class GroqAIService implements IAIService {
    ...
    async parseUnstructuredText(text: string, fromEmail?: string): Promise<IAIParsedMail> {
        try {
            console.log(`Sending text to Groq for parsing: ${text.substring(0, 50)}...`);

            // Derive a proper display name from the email address
            // e.g. "nithindaskavungal@gmail.com" → "Nithindaskavungal"
...
EXAMPLE "body" output:
───────────────────────────────────────────
"<p>Dear Sir/Madam,</p><p>I am writing to formally request a medical leave of absence for a period of <strong>4 days</strong>, commencing from <strong>Wednesday, February 26, 2026</strong>.</p><p>This leave is required due to a personal health matter that necessitates immediate medical attention. I will ensure that all pending assignments are completed or properly delegated prior to my absence to avoid any disruption to the team.</p><p>I kindly request your approval at the earliest convenience. Please feel free to reach out should you require any supporting documentation or further details from my end.</p><p>Thank you for your time and consideration.</p><p>Yours sincerely,<br/>${senderName}</p>"
───────────────────────────────────────────
